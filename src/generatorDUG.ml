...
let is_string x =
  match DUG.get_vinfo x |> fst with String -> true | _ -> false

let is_primitive x =
  match DUG.get_vinfo x |> fst with
  | Int | Long | Short | Byte | Float | Double | Bool | Char | String -> true
  | _ -> false

let is_primitive_from_v v =
  match get_type (fst v.DUGIR.variable) with
  | Int | Long | Short | Byte | Float | Double | Bool | Char | String -> true
  | _ -> false
...
let find_enum_var_list c_name i_info =
  ...
  | Some info ->
      List.fold_left
        (fun gvar_list const ->
          (0, DUGIR.GlobalConstant (c_name ^ "." ^ const)) :: gvar_list)
        [] info
  ...
  Condition.M.fold
    (fun head _ gvar_list ->
      ...
      | Condition.RH_Var var ->
          (0, DUGIR.GlobalConstant (DUG.get_short_class_name c_name ^ "." ^ var))
          :: gvar_list
      | _ -> gvar_list)
    s_trace []
...
let get_arg_seq num stmt tc (args : DUGIR.id list) =
      ...
      if List.mem arg !already_arg then
        ...
        let x =
          List.rev_append
            (List.fold_left
               (fun lst (ids, x) ->
                 (arg :: ids, DUG.mk_const_arg num arg x) :: lst)
               [] s)
            reuse
        in
        if is_primitive arg then x
          ...
          List.rev_append
            (List.fold_left
               (fun lst (ids, x) ->
                 (arg :: ids, DUG.mk_assign_arg num arg x) :: lst)
               [] s)
            x))
    [ ([], DUG.add_vertex (DUGIR.Skip num) DUG.empty) ]
    args

let mk_arg_seq num arg class_name s tc =
  let modified_x x =
    if is_primitive_from_v x then DUG.modify_import class_name x else x
  in
  get_arg_seq num s tc
    (List.fold_left
...
let const_unroll s p { summary; m_info; inst_info; prim_info; _ } =
  let get_r3 x =
    List.fold_left
      (fun lst x1 ->
        match snd x1 with
...
let fcall_in_assign_unroll s p obj_types
    { summary; m_info; t_info; c_info; setter_map; _ } =
  match s with
  | DUGIR.Assign (_, (x0, _, _, _)) ->
...
let recv_in_assign_unroll (prec, (s, tc), loop_ids, obj_types)
    { m_info; c_info; _ } =
  match s with
  | DUGIR.Assign (_, (_, _, f, arg)) when DUG.recv_in_assign s ->
      ...
      then (
        ...
        let applied =
          (prec, r2, loop_ids, obj_types) :: [ (prec, r3, loop_ids, obj_types) ]
        in
        if DUG.is_already_node ~is_recv:true s (fst r2) tc then
          let r4 = DUG.recv_in_assign_rule4 s (fst r2) tc in
          (prec, r4, loop_ids, obj_types) :: applied
        else applied
  | _ -> failwith "Fail: recv_in_assign_unroll"

let arg_in_assign_unroll (prec, (s, tc), loop_ids, obj_types) =
  match s with
  | DUGIR.Assign (num, (_, _, f, arg)) when DUG.arg_in_assign s ->
      let class_name = Utils.get_class_name (DUG.get_func f).method_name in
      mk_arg_seq (num + 1) arg class_name s tc
      |> List.fold_left
           (fun lst (args, x) ->
             ( prec,
               DUG.arg_in_assign_rule s x
                 (DUGIR.Param (List.map (fun x -> DUG.get_v x) args))
                 tc,
               loop_ids,
               obj_types )
             :: lst)
...
let void_unroll s p =
  let void_rule2 =
    List.fold_left
      (fun lst prev ->
        (0, DUG.void_rule2 prev s p.tc, empty_id_map, empty_obj_type_map) :: lst)
      [] (DUG.pred s p.tc)
  in
  (0, DUG.void_rule1 s p.tc, empty_id_map, empty_obj_type_map) :: void_rule2

let fcall_in_void_unroll s p p_data =
  match s with
  | DUGIR.Void (_, (x, _, _)) ->
      ...
      if lst = [] then raise Not_found_setter
        ...
        List.fold_left
          (fun lst (prec, f, arg) ->
            ( prec,
              DUG.fcall_in_void_rule s f (DUGIR.Arg arg) p.tc,
              empty_id_map,
              empty_obj_type_map )
            :: lst)
          [] lst
...
let recv_in_void_unroll (prec, (p, tc), loop_ids, obj_types)
    { m_info; c_info; _ } =
  match p with
  | DUGIR.Void (_, (_, f, _)) when DUG.recv_in_void p ->
      if cname_condition (DUG.get_func f).method_name m_info c_info then
        [
          (prec, DUG.recv_in_void_rule1 p (get_cname f) tc, loop_ids, obj_types);
        ]
      else
        let r2 = DUG.recv_in_void_rule2 p "con_recv" !recv tc in
        let r3 = DUG.recv_in_void_rule3 p "con_recv" !recv tc in
        incr recv;
        (prec, r2, loop_ids, obj_types) :: [ (prec, r3, loop_ids, obj_types) ]
  | _ -> failwith "Fail: recv_in_void_unroll"

let arg_in_void_unroll (prec, (s, tc), loop_ids, obj_types) =
  match s with
  | DUGIR.Void (num, (_, f, arg)) when DUG.arg_in_void s ->
      let class_name = Utils.get_class_name (DUG.get_func f).method_name in
      mk_arg_seq (num + 1) arg class_name s tc
      |> List.fold_left
           (fun lst (args, x) ->
...
let one_unroll s p obj_types p_data =
  match s with
  | _ when DUG.void p.tc s -> void_unroll s p
  | DUGIR.Const _ when DUG.const s -> const_unroll s p p_data
...
let combinate { prec; tc; loop_ids; obj_types; _ } stmt_map =
  let combinate_stmt (p', _, loop_ids', obj_types') s new_s_list =
    List.fold_left
...
let pretty_format tc =
  ...
  in
  let stmt_import s set =
    match s with
    | DUGIR.Const (_, (x, _)) -> add_import (DUG.get_v x).import set
    | Assign (_, (x0, _, f, arg)) ->
        List.fold_left
...
let mk_testcases ~is_start queue (e_method, error_summary) p_data =
  let apply_rule list =
    let start_node = DUGIR.Void (0, (Id, Func, Arg [])) in
